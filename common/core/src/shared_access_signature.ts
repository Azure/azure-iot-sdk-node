...
export class SharedAccessSignature {
  ...
  extend(expiry: number): string {
    ...
    this.se = expiry;
    this.sig = authorization.encodeUriComponentStrict(
      authorization.hmacHash(
        this._key,
        authorization.stringToSign(this.sr, this.se.toString())
      )
    );
  ...
  static create(
    ...
    sas.sig = authorization.encodeUriComponentStrict(
      authorization.hmacHash(
        sas._key,
        authorization.stringToSign(sas.sr, sas.se.toString())
      )
    );
    return sas;
  }
   ...
   * @private
   */
  static createWithSigningFunction(
    credentials: authorization.TransportConfig,
    expiry: number,
    signingFunction: Function,
    callback: (err: Error, sas?: SharedAccessSignature) => void
  ): void {
    function throwRef(name: string, value: any): void {
      throw new ReferenceError("Argument '" + name + "' is " + value);
    }
    ...
    if (credentials.sharedAccessKeyName) {
      sas.skn = authorization.encodeUriComponentStrict(
        credentials.sharedAccessKeyName
      );
    }
    signingFunction(
      Buffer.from(authorization.stringToSign(sas.sr, sas.se.toString())),
      (err, signed) => {
        if (err) {
