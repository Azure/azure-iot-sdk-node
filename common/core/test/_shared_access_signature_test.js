...
describe('SharedAccessSignature', function() {
  describe('#create', function() {
    ...
    function throwsWhenFalsy(argName) {
      ...
      function createSharedAccessSignature() {
        return SharedAccessSignature.create(
          args.resourceUri,
          args.keyName,
          args.key,
          args.expiry
    ...
    it('populates sig with the signature generated from resourceUri and expiry', function() {
      ...
      assert.propertyVal(
        sas,
        'sig',
        'kcWxXAK2OAaAoq2oCkaGzX2NMqDB8nCyNuq2fPYCDhk%3D'
      );
    });
  ...
  describe('#parse', function() {
    ...
    ['sr', 'sig', 'skn', 'se'].forEach(function(key) {
      it('throws if shared access signature is missing ' + key, function() {
        assert.throws(
          function() {
            SharedAccessSignature.parse(
              utils.makeStringWithout(key),
              utils.fields()
            );
          },
          ArgumentError,
          utils.errorMessage(key)
        );
      });
    });
  ...
  describe('#toString', function() {
    ...
    it('returns a signature token', function() {
      const expect =
        'SharedAccessSignature sr=uri&sig=kcWxXAK2OAaAoq2oCkaGzX2NMqDB8nCyNuq2fPYCDhk%3D&skn=encode%28%27Me%27%29&se=12345';
      const sas = SharedAccessSignature.create(
        'uri',
        "encode('Me')",
        key,
    ...
    it("does not include the skn segment if the skn property wasn't defined", function() {
      const expect =
        'SharedAccessSignature sr=uri&sig=kcWxXAK2OAaAoq2oCkaGzX2NMqDB8nCyNuq2fPYCDhk%3D&se=12345';
  ...
  describe('#createWithSigningFunction', function() {
    ...
    function throwsWhenFalsy(argName) {
      ...
      function _createWithSigningFunction() {
        return SharedAccessSignature.createWithSigningFunction(
          args.credentials,
          args.expiry,
          args.signingFunction,
          args.callback
        );
      }
    ...
    it('creates a valid sr value for device id', function(callback) {
      ...
      const fakeSigningFunction = sinon
        .stub()
        .callsArgWith(1, null, fakeSignedValue);
      SharedAccessSignature.createWithSigningFunction(
        fakeCredentials,
        1,
        fakeSigningFunction,
        (err, newSas) => {
          assert.strictEqual(
            newSas.sr,
            'fakeHostName%2Fdevices%2FfakeDeviceId',
            'Invalid scope create for sas token'
          );
    ...
    it('creates a valid sr value for device id and module id', function(callback) {
      ...
      SharedAccessSignature.createWithSigningFunction(
        ...
        (err, newSas) => {
          assert.strictEqual(
            newSas.sr,
            'fakeHostName%2Fdevices%2FfakeDeviceId%2Fmodules%2FfakeModuleId',
            'Invalid scope create for sas token'
          );
          assert.isUndefined(newSas.skn, 'Skn is improperly included');
          callback();
      ...
      SharedAccessSignature.createWithSigningFunction(
        ...
        (err, newSas) => {
          assert.strictEqual(
            newSas.se,
            1,
            'Invalid expiry create for sas token'
          );
          callback();
        }
      );
    ...
    it('Passes through an error from the signing function', function(callback) {
      ...
      const fakeSigningFunction = sinon
        .stub()
        .callsArgWith(1, new FormatError(), fakeSignedValue);
      SharedAccessSignature.createWithSigningFunction(
        fakeCredentials,
        1,
        fakeSigningFunction,
