...
const ExponentialBackOffWithJitter = require('../dist/retry_policy.js')
        ...
        assert.isFalse(
          testPolicy.shouldRetry(
            new errors.TimeoutError(
              'fake timeout that shall not be retried because of the filter'
            )
          )
        );
        assert.isTrue(
          testPolicy.shouldRetry(
            new errors.UnauthorizedError(
              'fake UnauthorizedError that shall be retried because of the filter'
            )
          )
        );
      });
    ...
    describe('nextRetryTimeout', function() {
      ...
      it('returns the proper value based on the retry count', function() {
        ...
        [
          ...
          assert.strictEqual(
            policy.nextRetryTimeout(testConfig.retryCount, false),
            testConfig.expected
          );
        });
        ...
        [
          ...
          assert.strictEqual(
            policy.nextRetryTimeout(testConfig.retryCount, true),
            testConfig.expected
          );
        });
        Math.random.restore();
