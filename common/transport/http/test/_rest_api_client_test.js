...
const HttpBase = require('../dist/http.js').Http;
const RestApiClient = require('../dist/rest_api_client.js').RestApiClient;

const fakeConfig = { host: 'host', sharedAccessSignature: 'sas' };
const socketConfig = { host: { socketPath: '/var/run/foo.sock' }, sharedAccessSignature: 'sas' };
const fakeAgent = 'agentString';
...
describe('RestApiClient', function () {
  describe('#constructor', function () {
    ...
    ['host'].forEach(function (badPropName) {
      [undefined, null, ''].forEach(function (badPropValue) {
        it('throws an ArgumentError if config.' + badPropName + ' is \'' + badPropValue + '\'', function () {
          const badConfig = JSON.parse(JSON.stringify(fakeConfig));
    ...
    it('uses \'azure-iot-common.Http\' if the \'httpBase\' argument is undefined', function () {
      const client = new RestApiClient(fakeConfig, fakeAgent);
    ...
    it('uses the \'httpBase\' argument if specified', function (testCallback) {
      ...
      const client = new RestApiClient(fakeConfig, fakeAgent, fakeHttpBase);
  ...
  describe('#executeApiCall', function () {
    ...
    it('adds default headers to every request', function (testCallback) {
      ...
      const client = new RestApiClient(fakeConfig, fakeAgent, fakeHttpHelper);
    ...
    it('uses the x509 cert if passed', function (testCallback) {
      const myFakeConfig = JSON.parse(JSON.stringify(fakeConfig));
      ...
      const client = new RestApiClient(myFakeConfig, fakeAgent, fakeHttpHelper);
    ...
    [fakeConfig, socketConfig].forEach((transportConfig) => {
      it('builds the HTTP request with the passed arguments', function (testCallback) {
        ...
        const testHeaderKey = 'Content-Type';
        const testHeaderValue = 'text/plain; charset=utf-8';
        const testHeaders = {};
        testHeaders[testHeaderKey] = testHeaderValue;
        const testRequestBody = 'foo';
        ...
        const client = new RestApiClient(transportConfig, fakeAgent, fakeHttpHelper);
    ...
    it('calls the done callback with a SyntaxError, an undefined result and a response if the request succeeds and is NOT valid JSON', function (testCallback) {
      ...
      const fakeResponseBody = "{ \"status\": 200, \"payload\": { }";
    ...
    it('calls the done callback with a translated error if the request fails with an HTTP error', function (testCallback) {
      const fakeResponse = { statusCode: 401 };
    ...
    it('Another calls the done callback with a translated error if the request fails with an HTTP error ', function (testCallback) {
      const fakeError = new Error('Non-HTTP error');
      const fakeHttpHelper = {
        buildRequest: function (method, path, headers, host, requestCallback) {
          return {
            setTimeout: function () {},
    ...
    [
      ...
      it('sets the Content-Length header when Content-Type is \'' + testConfig.contentType + '\' and content is ' + JSON.stringify(testConfig.content), function (testCallback) {
        ...
        const testRequestBody = testConfig.content;
        const expectedContentLength = (typeof testRequestBody === 'string') ? Buffer.byteLength(testRequestBody, 'utf8') : Buffer.byteLength(JSON.stringify(testRequestBody), 'utf8');
        const fakeHttpHelper = {
          buildRequest: function (method, path, headers, host, requestCallback) {
  ...
  describe('translateError', function () {
    [
      ...
      it('returns a \'' + testParams.expectedErrorType.name + '\' if the response status code is \'' + testParams.statusCode + '\'', function (){
        ...
        */
        const err = RestApiClient.translateError(fakeResponseBody, fakeResponse);
  ...
  describe('#setOptions', function () {
    ...
    it ('passes the options down', function (callback) {
      const fakeHttpRequestBuilder = { setOptions: sinon.stub() };
      const fakeOptions = '__FAKE_OPTIONS__';
      const client = new RestApiClient({ host: 'host', sharedAccessSignature: 'sas' }, fakeAgent, fakeHttpRequestBuilder);
