...
export class Http extends EventEmitter {
  ...
  constructor(idScope: string, registrationId: string, httpBase?: Base) {
    ...
    this._fsm = new machina.Fsm({
      ...
      states: {
        idle: {
          _onEnter: (callback, err, response) => {
          ...
          _onEnter: (callback) => {
            this._sendRegistrationRequest((err, responseBody) => {
              if (this._registrationInProgress()) { // make sure we weren't cancelled before doing something with the response
                if (err) {
                ...
                } else if (responseBody.status.toLowerCase() === 'assigning') {
                  ...
                  this._operationId = responseBody.operationId;
                  this._fsm.transition('waiting_to_poll', callback);
                } else if (responseBody.status.toLowerCase() === 'assigned') {
                  ...
                  this._fsm.transition('idle', callback, null, responseBody);
                } else {
  ...
  private _sendRegistrationRequest(callback: (err: Error, body?: any) => void): void {
    ...
    /* Codes_SRS_NODE_PROVISIONING_HTTP_18_010: [ `register` shall wait for the response to the PUT request. ] */
