...
describe('Amqp', function () {
  ...
  describe('TPM', function() {
    ...
    before(function() {
      ...
      sinon.stub(SaslTpm.prototype, 'getInitFrameContent');
      sinon.stub(SaslTpm.prototype, 'getChallengeResponseContent');
    });

    after(function() {
      SaslTpm.prototype.getInitFrameContent.restore();
      SaslTpm.prototype.getChallengeResponseContent.restore();
      SaslTpm.restore();
      SaslTpm = null;
    })
    ...
    describe('getAuthenticationChallenge', function() {
      ...
      it ('returns success correctly', function(callback) {
        amqp.getAuthenticationChallenge(fakeRequest, function(err, challenge) {
          ...
          assert.isNull(err);
          assert.strictEqual(challenge.authenticationKey, fakeAuthenticationKey);
          callback();
        });
