...
const deviceConnectionString = 'HostName=localhost;DeviceId=alpha;SharedAccessKey=FakeAccessKey';
...
function generateMessage () {
  const message = new Message(JSON.stringify({ "temperature" : Math.random() * 100 }));
  return message;
}
...
function logResultFor(op, messageNumber) {
  return function printResult(err, res) {
    ...
    if (res) {
      ...
      const metrics = { 'messageNumber': messageNumber, 'responseTime': responseTime[messageNumber] }
      fs.appendFile(
        filePath,
      ...
      process.send({ 'messageNumber': messageNumber, 'responseTime': responseTime[messageNumber] })
