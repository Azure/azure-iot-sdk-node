...
export class DiagnosticClient {
  ...
  static DIAGNOSTIC_TWIN_KEY_SAMPLING_PERCENTAGE: string = '_diag_sample_rate';
  static DIAGNOSTIC_TWIN_KEY_ERROR_MSG: string = '_diag_info';
   ...
   */
  constructor() {
    /* Codes_SRS_NODE_DEVICE_DIAGNOSTICCLIENT_01_001: [The Client constructor shall initial the percentage and message count.] */
   ...
   * @param {any}    samplingRate    The sampling rate
   */
  onDesiredTwinUpdate(twin: Twin, samplingRate: any): void {
    ...
    // We would like to handle all value types for samplingRate here, if the type is not number we will report in the twin.
    // That's why the samplingRate type is any.
    if (typeof samplingRate === 'number') {
      try {
        this.setDiagSamplingPercentage(samplingRate);
      } catch (e) {
        errMsg += `Value of ${DiagnosticClient.DIAGNOSTIC_TWIN_KEY_SAMPLING_PERCENTAGE} is invalid:${e.message} `;
      }
    } else if (samplingRate === null) {
      this.setDiagSamplingPercentage(0);
    } else {
      errMsg += `Value of ${DiagnosticClient.DIAGNOSTIC_TWIN_KEY_SAMPLING_PERCENTAGE} is invalid:${samplingRate}(check if it is number) `;
    }
