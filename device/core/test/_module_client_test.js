...
let IotEdgeAuthenticationProvider = require('../dist/iotedge_authentication_provider').IotEdgeAuthenticationProvider;
...
describe('ModuleClient', function () {
  let sharedKeyConnectionString = 'HostName=host;DeviceId=id;ModuleId=modId;SharedAccessKey=key';
  let sharedAccessSignature = '"SharedAccessSignature sr=hubName.azure-devices.net/devices/deviceId/modules/moduleId&sig=s1gn4tur3&se=1454204843"';
  ...
  describe('#fromConnectionString', function () {
    ...
    it('returns an instance of ModuleClient', function () {
      let client = ModuleClient.fromConnectionString(sharedKeyConnectionString, FakeTransport);
  ...
  describe('#fromSharedAccessSignature', function () {
    ...
    it('returns an instance of ModuleClient', function () {
      let client = ModuleClient.fromSharedAccessSignature(sharedAccessSignature, FakeTransport);
      assert.instanceOf(client, ModuleClient);
    });
  });
  ...
  describe('#fromAuthenticationProvider', function () {
    ...
    it('returns an instance of ModuleClient', function () {
      let client = ModuleClient.fromAuthenticationProvider({}, FakeTransport);
      assert.instanceOf(client, ModuleClient);
  ...
  describe('#fromEnvironment', function () {
    ...
    ['EdgeHubConnectionString', 'IotHubConnectionString'].forEach(function (envName) {
      describe('sets CA cert in non-edge mode', function () {
        let spy;
        let fsstub;
        let fakeConnStr = 'HostName=fake.azure-devices.net;DeviceId=deviceId;ModuleId=moduleId;SharedAccessKey=key';

        let transport = new EventEmitter();
    ...
    describe('check IOTEDGE_AUTHSCHEME', function () {
      let requiredVars = ['IOTEDGE_WORKLOADURI', 'IOTEDGE_DEVICEID', 'IOTEDGE_MODULEID', 'IOTEDGE_IOTHUBHOSTNAME', 'IOTEDGE_MODULEGENERATIONID'];

      let getTrustBundleStub;
    ...
    describe('trust bundle 2', function () {
      let env = [
        ['IOTEDGE_WORKLOADURI', 'unix:///var/run/iotedge.w.sock'],
        ['IOTEDGE_DEVICEID', 'd1'],
        ['IOTEDGE_MODULEID', 'm1'],
      ...
      it('fails if getTrustBundle fails', function (testCallback) {
        ...
        let transportStub = sinon.stub().returns(transport);
        ModuleClient.fromEnvironment(transportStub, function (err, _client) {
          assert.isOk(err);
          assert.strictEqual(err, 'whoops');
          assert.strictEqual(getTrustBundleStub.called, true);
  ...
  ['sendOutputEvent', 'sendOutputEventBatch'].forEach(function (funcName) {
    describe('#' + funcName, function () {
      ...
      it('doesn\'t throw if no callback is given and the method exists on the transport', function () {
        ...
        let client = new ModuleClient(transport);
  ...
  describe('#invokeMethod', function () {
    ...
    it('calls MethodClient.invokeMethod with the proper arguments when the target is a device', function () {
      ...
      let fakeMethodParams = {
        methodName: 'methodName'
      };
      let client = new ModuleClient(transport, fakeMethodClient);
  ...
  describe('#on(\'inputMessage\')', function () {
    ...
    it('emits a message event when a message is received', function (done) {
      ...
      let client = new ModuleClient(fakeTransport);
  ...
  describe('#setOptions', function () {
    ...
    [null, undefined].forEach(function (options) {
      it('throws is options is ' + options, function () {
        let client = new ModuleClient(new EventEmitter(), fakeMethodClient);
        assert.throws(function () {
    ...
    it('calls the done callback with no parameters when it has successfully configured the transport', function (done) {
      let client = new ModuleClient(new FakeTransport(), fakeMethodClient);
    ...
    it('calls the done callback with an error when it failed to configured the transport', function (done) {
      ...
      let client = new ModuleClient(failingTransport, fakeMethodClient);
    ...
    it('calls setOptions on the MethodClient', function () {
      ...
      let fakeOptions = {};
      let client = new ModuleClient(new FakeTransport(), methodClient);
  ...
  describe('#onMethod', function () {
    ...
    it('calls response send with 400 status when method call contains invalid body', function (done) {
      ...
      let responseSpy = sinon.spy(transport, 'sendMethodResponse');
      // test
