...
const { AnonymousCredential, uploadStreamToBlockBlob, Aborter, BlobURL, BlockBlobURL, ContainerURL, ServiceURL, StorageURL } = require('@azure/storage-blob');
const { Mqtt: Protocol } = require('azure-iot-device-mqtt');
const { Client } =  require('azure-iot-device');
const { createReadStream, unlinkSync, writeFileAsync, stat } = require('fs');
...
const run = async() => {
    try {
        ...
        err = await to(client.notifyBlobUploadStatus(blobInfo.correlationId, isSuccess, statusCode, statusDescription));
        if (err) { console.error('notifyBlobUploadStatus failed'); } else { console.log('notifyBlobUploadStatus success'); }
