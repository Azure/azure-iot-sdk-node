...
export class Amqp extends EventEmitter implements Client.Transport {
  ...
  sendToModuleInput(deviceId: string, moduleId: string, inputName: string, message: Message, done?: Callback<results.MessageEnqueued>): void {
    const moduleEndpoint = endpoint.moduleMessagePath(encodeURIComponent(deviceId), encodeURIComponent(moduleId));
    let amqpMessage = AmqpMessage.fromMessage(message);
    ...
    this._fsm.handle('send', amqpMessage, moduleEndpoint, handleResult('AMQP Transport: Could not send message', done));
  }
