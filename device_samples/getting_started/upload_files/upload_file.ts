...
if (filePath === '' || filePath === undefined) {
  console.error(logRed, 'Missing path to file string');
  process.exit(0);
}
...
async function uploadToBlob(localFilePath: string, client: Client): Promise<void> {
  ...
  try {
    ...
    console.error(logRed, `notifyBlobUploadStatus failed: ${err.message}`);
  }
...
uploadToBlob(filePath, deviceClient)
  .catch((err: Error) => {
    console.error(logRed, `Upload to blob failed: ${err.message}`);
  })
  .finally(() => {
    process.exit(0);
  });