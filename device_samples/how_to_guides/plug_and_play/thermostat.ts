...
let deviceConnectionString: string = process.env.IOTHUB_DEVICE_CONNECTION_STRING ?? '';
...
async function provisionDevice(payload: any): Promise<void> {
  const provSecurityClient: SymmetricKeySecurityClient = new SymmetricKeySecurityClient(registrationId, symmetricKey);
