...
const edgeCaCertPath: string = process.env.PATH_TO_CERTIFICATE_FILE ?? '';
...
let connectCallback = function (err: Error | any): void {
  if (err) {
    ...
    client.on('error', function (err: Error | any): void {
...
client.setOptions(options, function(err: Error | any): void {
  if (err) {
    console.log(logRed, 'SetOptions Error: ' + err);
  } else {
    client.open(connectCallback);
  }
});
...
function printResultFor(op: any): (err: Error | any, res: any) => void {
